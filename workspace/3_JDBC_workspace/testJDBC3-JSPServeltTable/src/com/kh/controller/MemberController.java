...
public class MemberController {
	...
	public int insertMember() {
		...
		MemberJSPTable mem = menu.insertMember();  // 자동완성 시, 반환타입이 Member로 자동적으로 채워짐
		// 
...
//		int result = mDAO.insertMember(mem, conn); // 인서트멤버에 빨간줄 났던 이유 : DAO의 메소드인 insertMember(Member mem)에서
	...
	public void selectAll() {
		...
		ArrayList<MemberJSPTable> list = mService.selectAll();
...
//			menu.selectAll(list); // 숙제로 내가 한 코드
	...
	public void selectMember() {
		...
		switch(sel) {
		case 1: 
				String id = menu.inputUserId();
				list = mService.selectUserId(id);	// 패턴만 맞아도 값을 가져올 것임. 패턴 하나만 맞아도  된다고 했고 여러명일 수 있기에 
				break;								// ArrayList
		case 2: 
				String nickname = menu.inputNickname();		
				list = mService.selectNickname(nickname);	// ArrayList
	...
	public void updateMember() {
		...
		String memberId = menu.inputUserId(); // 받아온 아이디가 존재하는지 확인절차 필요. db에서 확인해야하니 service에 연결
	...
	public void deleteMember() {
		...
		String id = menu.inputUserId(); // 삭제할 아이디 입력 받기
