	...
	<div class="outer">
		...
		<form action="<%= request.getContextPath() %>/insert.th" method="post" encType="multipart/form-data"><!-- 파일올리는 거기 때문에 -->
			<div class="insertArea">
				...
				<!-- 파일 업로드 하는 부분 -->
				<div id="fileArea">
					<input type="file" id="thumbnailImg1" multiple="multiple" name="thumbnailImg1" onchange="LoadImg(this,1)"> <!-- this 이 태그 전체를 가리킴 -->
				...
				<script>
					...
					$(function(){
						$("#fileArea").hide(); /* 위의 div태그를 숨겨두는 .hide() */
					...
					function LoadImg(value, num){
						if(value.files && value.files[0]){ /* 왜 value.files[0]까지 해야할까? // 파일 복수선택 가능하게 되어있는 상태인데 파일 열기하면 1번쨰 파일만 대표이미지에 들어감 */
							var reader = new FileReader();
							
							reader.onload = function(e){								
								switch(num){
								case 1: 
									$("#titleImg").attr("src", e.target.result); /* src속성을 e.target.result로  바꿔줘라. */
									break;
								case 2:
							...
							reader.readAsDataURL(value.files[0]); // 파일꺼임// 데이터 파일 경로 // 파일 인지할 수 있게하는 경로지정(?) 12:38
						}
					}
				</script>
